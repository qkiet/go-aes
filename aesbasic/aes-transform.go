package aesbasic

import "github.com/qkiet/go-aes/common"

func sbox(a byte) byte {
	sboxTable := [256]byte{
		,
	}
	return sboxTable[a]
}

func invSbox(a byte) byte {
	invBoxTable := [256]byte{
		,
	}
	return invBoxTable[a]
}

func rotWord(b common.AesWord) common.AesWord {
	ret := common.AesWord{
		b[1], b[2], b[3], b[0],
	}
	return ret
}

func subWord(b common.AesWord) common.AesWord {
	ret := common.AesWord{
		sbox(b[0]), sbox(b[1]), sbox(b[2]), sbox(b[3]),
	}
	return ret
}

func KeyExpansion(key []byte) []byte {
	var roundNumber int
	var expandedKeySize int
	var keyLength int
	rConConstants := [10]common.AesWord{
		{0x01, 0x00, 0x00, 0x00},
		{0x02, 0x00, 0x00, 0x00},
		{0x04, 0x00, 0x00, 0x00},
		{0x08, 0x00, 0x00, 0x00},
		{0x10, 0x00, 0x00, 0x00},
		{0x20, 0x00, 0x00, 0x00},
		{0x40, 0x00, 0x00, 0x00},
		{0x80, 0x00, 0x00, 0x00},
		{0x1b, 0x00, 0x00, 0x00},
		{0x36, 0x00, 0x00, 0x00},
	}
	switch len(key) {
	case int(common.AesKeySize_Aes128):
		roundNumber = 10
		expandedKeySize = 4 * (roundNumber + 1)
		keyLength = 4
	}
	// because size of key is already checked, no need to check error here
	keyWords, _ := common.BytesToWords(key)
	ret := make([]common.AesWord, expandedKeySize)
	copy(ret[0:keyLength], keyWords[:])
	for i := 4; i <= 4*roundNumber+3; i++ {
		var tmp = ret[i-1]
		if i%keyLength == 0 {
			tmp = GF_AddWord(subWord(rotWord(tmp)), rConConstants[i/keyLength-1])
		} else if keyLength > 6 && i%keyLength == 4 {
			tmp = subWord(tmp)
		}
		ret[i] = GF_AddWord(ret[i-keyLength], tmp)
	}
	return common.WordsToBytes(ret)
}

func KeyExpansionEic(key []byte) []byte {
	expandedKey := KeyExpansion(key)
	var roundNumber int
	switch len(key) {
	case int(common.AesKeySize_Aes128):
		roundNumber = 10
	}

	for r := 1; r <= roundNumber-1; r++ {
		iBytes := r * 16
		// We only put 16 bytes, so it's guaranteed to be successfully get 4 words
		expandKeyWords, _ := common.BytesToWords(expandedKey[iBytes:(iBytes + 16)])
		// We only put 4 words, so it's guaranteed to be successful
		inputState, _ := common.WordsToAesState(expandKeyWords[:])
		outputState := InvMixColumns(inputState)
		outputBytes := common.AesStateToBytes(outputState)
		copy(expandedKey[iBytes:(iBytes+16)], outputBytes[:])
	}
	return expandedKey
}

func AddRoundKey(s, key common.AesState) common.AesState {
	return common.AesState{
		{s[0][0] ^ key[0][0], s[0][1] ^ key[0][1], s[0][2] ^ key[0][2], s[0][3] ^ key[0][3]},
		{s[1][0] ^ key[1][0], s[1][1] ^ key[1][1], s[1][2] ^ key[1][2], s[1][3] ^ key[1][3]},
		{s[2][0] ^ key[2][0], s[2][1] ^ key[2][1], s[2][2] ^ key[2][2], s[2][3] ^ key[2][3]},
		{s[3][0] ^ key[3][0], s[3][1] ^ key[3][1], s[3][2] ^ key[3][2], s[3][3] ^ key[3][3]},
	}
}

func SubBytes(s common.AesState) common.AesState {
	return common.AesState{
		{sbox(s[0][0]), sbox(s[0][1]), sbox(s[0][2]), sbox(s[0][3])},
		{sbox(s[1][0]), sbox(s[1][1]), sbox(s[1][2]), sbox(s[1][3])},
		{sbox(s[2][0]), sbox(s[2][1]), sbox(s[2][2]), sbox(s[2][3])},
		{sbox(s[3][0]), sbox(s[3][1]), sbox(s[3][2]), sbox(s[3][3])},
	}
}

func ShiftRows(s common.AesState) common.AesState {
	return common.AesState{
		{s[0][0], s[0][1], s[0][2], s[0][3]},
		{s[1][1], s[1][2], s[1][3], s[1][0]},
		{s[2][2], s[2][3], s[2][0], s[2][1]},
		{s[3][3], s[3][0], s[3][1], s[3][2]},
	}
}

func MixColumns(s common.AesState) common.AesState {
	var ret common.AesState
	for i := 0; i < 4; i++ {
		ret[0][i] = GF_Multiply(0x2, s[0][i]) ^ GF_Multiply(0x3, s[1][i]) ^ s[2][i] ^ s[3][i]
	}
	for i := 0; i < 4; i++ {
		ret[1][i] = s[0][i] ^ GF_Multiply(0x2, s[1][i]) ^ GF_Multiply(0x3, s[2][i]) ^ s[3][i]
	}
	for i := 0; i < 4; i++ {
		ret[2][i] = s[0][i] ^ s[1][i] ^ GF_Multiply(0x2, s[2][i]) ^ GF_Multiply(0x3, s[3][i])
	}
	for i := 0; i < 4; i++ {
		ret[3][i] = GF_Multiply(0x3, s[0][i]) ^ s[1][i] ^ s[2][i] ^ GF_Multiply(0x2, s[3][i])
	}
	return ret
}

func InvShiftRows(s common.AesState) common.AesState {
	return common.AesState{
		{s[0][0], s[0][1], s[0][2], s[0][3]},
		{s[1][3], s[1][0], s[1][1], s[1][2]},
		{s[2][2], s[2][3], s[2][0], s[2][1]},
		{s[3][1], s[3][2], s[3][3], s[3][0]},
	}
}

func InvSubBytes(s common.AesState) common.AesState {
	return common.AesState{
		{invSbox(s[0][0]), invSbox(s[0][1]), invSbox(s[0][2]), invSbox(s[0][3])},
		{invSbox(s[1][0]), invSbox(s[1][1]), invSbox(s[1][2]), invSbox(s[1][3])},
		{invSbox(s[2][0]), invSbox(s[2][1]), invSbox(s[2][2]), invSbox(s[2][3])},
		{invSbox(s[3][0]), invSbox(s[3][1]), invSbox(s[3][2]), invSbox(s[3][3])},
	}
}

func InvMixColumns(s common.AesState) common.AesState {
	var ret common.AesState
	for i := 0; i < 4; i++ {
		ret[0][i] = GF_Multiply(0x0e, s[0][i]) ^ GF_Multiply(0x0b, s[1][i]) ^ GF_Multiply(0x0d, s[2][i]) ^ GF_Multiply(0x09, s[3][i])
	}
	for i := 0; i < 4; i++ {
		ret[1][i] = GF_Multiply(0x09, s[0][i]) ^ GF_Multiply(0x0e, s[1][i]) ^ GF_Multiply(0x0b, s[2][i]) ^ GF_Multiply(0x0d, s[3][i])
	}
	for i := 0; i < 4; i++ {
		ret[2][i] = GF_Multiply(0x0d, s[0][i]) ^ GF_Multiply(0x09, s[1][i]) ^ GF_Multiply(0x0e, s[2][i]) ^ GF_Multiply(0x0b, s[3][i])
	}
	for i := 0; i < 4; i++ {
		ret[3][i] = GF_Multiply(0x0b, s[0][i]) ^ GF_Multiply(0x0d, s[1][i]) ^ GF_Multiply(0x09, s[2][i]) ^ GF_Multiply(0x0e, s[3][i])
	}
	return ret
}
